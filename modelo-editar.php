...
<?php
    ...
    $res_1 = $conn->query($sql_1); // Corrigido de querry para query
    $row_1 = $res_1->fetch_object();
?>
<form action="?page=modelo-salvar" method="POST">
    <input type="hidden" name="acao" value="editar">
    <div class="mb-3"> <!-- Corrigido de mb=3 para mb-3 -->
        <label>Marca</label>
        <select name="marca_id_marca" class="form-control">
            <option>- Escolha -</option> <!-- Corrigido de ption para option -->
            <?php
                ...
                if($res->num_rows > 0){
                    while($row = $res->fetch_object()){
                        if($row_1->marca_id_marca == $row->id_marca){
                            ...
                            print "<option value='".$row->id_marca."'>".$row->nome_marca."</option>"; // Corrigido de prtint para print
                        }
                    }
                } else {
                    print "<option>Não há mais marcas cadastradas!</option>";
                }
            ?>
        </select>
    </div>
